...
``` r
# oprional: if u want to see the dropped replicates
feature_norm %>%
  filter(PROTEIN %in% synaptic_proteins) %>%
  group_by(originalRUN) %>%
...
feature_norm <- feature_norm %>%
  ...
  mutate(
    # SEGURIDAD: Si una muestra no tiene factor sináptico, 
    # mantenemos su intensidad original en lugar de convertirla en NA
...
``` r
# Renombrar para que coincida con el formato de entrada de MSstats
data_for_summarization <- processed$FeatureLevelData %>%
  rename(
    ProteinName = PROTEIN ,
    PeptideSequence = PEPTIDE,
    # MSstats requiere estas dos aunque sean iguales tras el primer procesado
    PeptideModifiedSequence = PEPTIDE, 
    PrecursorCharge = FEATURE, # Usamos FEATURE que combina péptido y carga
    IsotopeLabelType = LABEL,
    Condition = GROUP,
    BioReplicate = SUBJECT,
    Run = RUN,
    # Usamos tu 'Intensity' personalizada (la ajustada sinápticamente)
    Intensity = Intensity 
  ) %>%
  mutate(
    FragmentIon = "NA",
...
``` r
# 1. Preparamos los datos usando la columna GROUP que ya existe
pca_data <- protein_data %>%
  dplyr::select(Protein, originalRUN, LogIntensities, GROUP) %>%
  # GROUP ya contiene "AD" y "CN_PART" porque lo modificamos antes
  pivot_wider(names_from = Protein, values_from = LogIntensities) %>%
  # Imputación de NAs con la media
  mutate(across(-c(originalRUN, GROUP), 
                ~ ifelse(is.na(.), mean(., na.rm = TRUE), .)))

# 2. Verificación de grupos
print("Conteo de grupos en la columna GROUP:")
```

    ## [1] "Conteo de grupos en la columna GROUP:"
...
``` r
# 3. Ejecutar PCA (excluimos las columnas de texto originalRUN y GROUP)
pca_res <- prcomp(pca_data %>% dplyr::select(-originalRUN, -GROUP), scale. = TRUE)

# 4. Graficar
as.data.frame(pca_res$x) %>%
  mutate(Condition = pca_data$GROUP) %>%
  ggplot(aes(PC1, PC2, color = Condition)) +
  geom_point(size = 4) +
  stat_ellipse() +
  theme_bw() +
  # Usamos los nombres exactos que aparecen en tu tabla (probablemente AD y CN_PART)
  scale_color_manual(values = c("AD" = "#F8766D", "CN_PART" = "#00BFC4")) +
  labs(title = "PCA Proteómico (Normalización Sináptica)",
       subtitle = paste("AD:", sum(pca_data$GROUP == "AD"), 
                        "| CN_PART:", sum(pca_data$GROUP == "CN_PART")),
       x = paste0("PC1 (", round(summary(pca_res)$importance[2,1]*100,1),"%)"),
       y = paste0("PC2 (", round(summary(pca_res)$importance[2,2]*100,1),"%)"))
```

<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-16-1.png" alt="" style="display: block; margin: auto;" />

``` r
ggsave("PCA_Proteomico_Final.png", plot = last_plot(), device = "png", width = 20, height = 15, units = "cm", dpi = 300)
```

## 14. Differential testing no covariates
...
# Matriu de contrastos per AD vs CN_PART
...
# Assignar noms de columnes exactes
colnames(contrast_matrix) <- c("AD", "CN_PART")

# Assignar nom de la fila
...
```
    ...
    ## INFO  [2026-05-27 18:41:11]  == Start to test and get inference in whole plot ...
    ...
    ## INFO  [2026-05-27 18:41:46]  == Comparisons for all proteins are done.
...
# 1. Obtener Top 10 Hits
top_10_hits <- plot_data %>% 
  filter(!is.na(pvalue)) %>% 
  arrange(pvalue) %>% 
  head(10)

# Exportar el top 10 de proteínas a un archivo Excel
library(openxlsx)
write.xlsx(top_10_hits, "Top_10_Proteins_Volcano_unadjusted.xlsx", rowNames = FALSE)


# 2. Volcano Plot corregido para que no se corte la etiqueta superior
volcano_final <- ggplot(plot_data, aes(x = log2FC, y = log_p, color = status)) +
  ...
  # Expandimos el límite de Y a 17 para dar aire arriba
  ...
  geom_text_repel(
    ...
    color = "black", 
    fontface = "italic", 
    box.padding = 0.8,      # Un poco más de espacio
    segment.size = 0.2,
    max.overlaps = Inf,
    # Forzamos a que las etiquetas intenten quedarse dentro del área
    force = 2
  ) +
  labs(
  ...
  # CRÍTICO: Esto evita que se corten los nombres que toquen los bordes
  ...
  theme(
    ...
    # Añadimos un poco de margen superior extra al gráfico completo
    plot.margin = margin(t = 20, r = 10, b = 10, l = 10)
  )

# Guardar
ggsave("Volcano_Final_SinCortes.png", plot = volcano_final, width = 15, height = 15, units = "cm", dpi = 300)
...
``` r
## 15. Heatmap de nivel publicación (Solo Nombres y Condición)
library(pheatmap)
library(tidyr)
library(dplyr)
library(tibble)
...
# 2. Generar el heatmap con la leyenda correcta
pheatmap(
  heatmap_matrix, 
  scale = "row",
  clustering_distance_rows = "euclidean",
  ...
  # --- CAMBIO AQUÍ: Nombre técnico de la escala ---
  name = "Z-score", 
  # -----------------------------------------------
...
# 2. Definir el contraste de interés (AD vs Control)
# Asegúrate de usar los nombres exactos que aparecen en colnames(design)
cont_matrix <- makeContrasts(
  AD_vs_Control = GROUPAD - GROUPCN_PART, 
  levels = design
)

# 3. Aplicar el contraste y calcular estadísticas moderadas (eBayes)
fit2 <- contrasts.fit(fit, cont_matrix)
fit2 <- eBayes(fit2)

# 4. Generar la tabla de resultados finales
# Se usa el método BH (Benjamini-Hochberg) para controlar el FDR
top_table <- topTable(fit2, coef = "AD_vs_Control", number = Inf, adjust.method = "BH")

# Mostrar las proteínas más significativas ajustadas por covariables
head(top_table)
```
...
# Top 10 para etiquetas
top_labels <- results_adj_plot %>%
  filter(Status != "Not Significant") %>%
  arrange(P.Value) %>%
  head(10)

# --- 3. GRÁFICO CON ESTILO "PROTEOMIC DYSREGULATION" ---
volcano2 <- ggplot(results_adj_plot, aes(x = logFC, y = -log10(P.Value), color = Status)) +
  # Puntos con la misma transparencia y tamaño del primer gráfico
  geom_point(alpha = 0.5, size = 1.2) +
  
  # Paleta de colores idéntica (Azul y Naranja vibrante)
  scale_color_manual(values = c(
    "Down-regulated" = "#0072B2", 
    "Up-regulated" = "#D55E00", 
    "Not Significant" = "gray85"
  ...
  # Etiquetas con fuente italic y líneas finas
  geom_text_repel(
  ...
  # Líneas de corte punteadas y finas
  geom_vline(xintercept = c(-0.5, 0.5), linetype = "dotted", color = "gray40", size = 0.3) +
  geom_hline(yintercept = -log10(0.05), linetype = "dotted", color = "gray40", size = 0.3) +
  
  # Centrado del eje X y límites
  scale_x_continuous(limits = c(-6, 6)) + 
  
  # Estética limpia (Theme BW + ajustes)
  theme_bw() +
  labs(
    title = "Proteomic Dysregulation",
  ...
  theme(
    aspect.ratio = 1,                 # Fuerza el formato CUADRADO
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(),
    legend.position = "top",          # Leyenda arriba como en el primer gráfico
...
# --- 4. GUARDAR ---
# Guardamos con dimensiones iguales para mantener el cuadrado perfecto
...
# 1. Construcción de la matriz desde processed_final
# =====================================================
...
# =====================================================
# 2. label_map (igual que en pheatmap)
# =====================================================
...
# 4. Metadatos usando design_meta2
...
annotation_table <- annotation_table %>%
  rename(
    Protein = Protein,   # ajustar si ya lo tienes así
    Gene = Gene
  )
...
annotation_table <- annotation_table %>%
  rename(
    Protein = Protein,
    Gene = Gene,
    Protein_name = Protein.names   # ajusta si el nombre es distinto en tu archivo
  )
...
## =========================================================
## 4. TOTAL SIGNIFICANT PROTEINS (PI REQUIREMENT)
## =========================================================
...
# 1. Creamos la columna Protein y definimos el estatus igual que en el gráfico
...
# 2. Filtramos para obtener exactamente los "Top Hits" del gráfico
# Seleccionamos solo las que tienen P-valor < 0.05 y las 10 más significativas
...
# Guardamos el objeto top_10_hits
...
# 1. Identificamos todas las proteínas coloreadas (Up y Down)
# Usamos exactamente los mismos umbrales que el gráfico: P.Value < 0.05 y |logFC| > 0.5
all_significant_colored <- top_table %>%
  mutate(Protein = rownames(.)) %>%
  filter(P.Value < 0.05 & abs(logFC) > 0.5) %>%
  # Añadimos la columna Status para que en el Excel veas si es Up o Down
  mutate(Status = if_else(logFC > 0, "Up-regulated", "Down-regulated")) %>%
  # Ordenamos primero por el P-valor para tener las más potentes arriba
  arrange(P.Value)

# 2. Guardar en formato Excel
file_name_all <- paste0("All_Colored_Proteins_", Sys.Date(), ".xlsx")

# Guardamos la lista completa de proteínas coloreadas
write.xlsx(all_significant_colored, file = file_name_all, rowNames = FALSE)

# Mensaje de confirmación con el conteo
message("¡Éxito! Se han guardado ", nrow(all_significant_colored), 
        " proteínas coloreadas en el archivo: ", file_name_all)
```

## 16. Correlación con patología
...
# --- STEP 1: Importación y Limpieza de Metadatos ---
metadata_patho <- read_excel("experimental_annotationfull.xlsx")

# Creamos un mapeo basado en el orden real de tus muestras (originalRUN 1 a 18)
...
design_patho <- metadata_patho %>%
  mutate(
    # Sincronizamos usando la lista exacta de muestras
    # originalRUN en MSstats suele ser el índice (1, 2, 3...)
...
# --- STEP 2: Preparación de la Matriz de Proteínas ---
# IMPORTANTE: Usamos originalRUN porque son los IDs (1-18) que MSstats guarda internamente
...
# --- STEP 3: Sincronización Final ---
# Cruce por los números de muestra (1, 2, 3...)
...
# --- STEP 4: Modelos Lineales Ajustados (Los tres estudios) ---
...
# 3. MODELO INTERACCIÓN (Braak x Thal)
...
# Buscamos el coeficiente de interacción (el último generado habitualmente)
...
``` r
# 1. Definición de marcadores (Asegúrate de que estos UniprotID estén en tu dataset)
...
# 3. Gráfico con Estética de Publicación
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-38-1.png" alt="" style="display: block; margin: auto;" />
...
# 1. Definición de muestras basada en el análisis exitoso anterior
# Extraemos los IDs que realmente están en tu matriz final (n=18)
mis_muestras <- colnames(data_matrix_final)

# 2. Función de preparación robusta
# Esta función detecta si el objeto es la matriz "ancha" o el dataframe "largo"
```
...
prep_data_qc <- function(obj, name) {
  ...
  # Si después de limpiar no hay filas, el objeto estaba vacío
  if(nrow(df_plot) == 0) stop(paste("El objeto", name, "no tiene datos válidos."))

  # Aplicar Log2 solo si los datos están en escala lineal (mayores a 1000)
...
# Colores escalonados de azul (Estética Nature)
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-42-1.png" alt="" style="display: block; margin: auto;" />
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-51-1.png" alt="" style="display: block; margin: auto;" />
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-54-1.png" alt="" style="display: block; margin: auto;" />
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-54-2.png" alt="" style="display: block; margin: auto;" />
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-56-1.png" alt="" style="display: block; margin: auto;" />

``` r
output_df_CC <- run_pathfindR(processed_pathfindR[,c(2,4)], gene_sets = "GO-CC")
```

<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-56-2.png" alt="" style="display: block; margin: auto;" />
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-58-2.png" alt="" style="display: block; margin: auto;" />
...
<img src="TFG_Mar_Mauri_files/figure-gfm/unnamed-chunk-61-1.png" alt="" style="display: block; margin: auto;" />
...
# 1. Definimos la lista en el orden exacto (del 1 al 18)
# Esta lista asocia el número de fila/run con su nombre real
muestras_lista <- c(
  "Intensity.AD3",   # 1
  "Intensity.AD6",   # 2
  "Intensity.AD11",  # 3
...
# 2. Preparamos los datos usando el índice numérico de originalRUN
data_para_grafico <- processed_final$ProteinLevelData %>%
  mutate(
    # Convertimos originalRUN a número para usarlo como índice de la lista
    ID_numerico = as.numeric(as.character(originalRUN)),
    # Obtenemos el nombre completo (ej. Intensity.CN18)
    FullName = muestras_lista[ID_numerico],
    # Limpiamos el nombre para el gráfico (ej. Control 18)
    SampleLabel = FullName %>% 
      str_replace("Intensity.CN", "Control ") %>% 
      str_replace("Intensity.PT", "PART ") %>%
      str_replace("Intensity.AD", "AD "),
    # Asignamos el Grupo final
    Group = case_when(
  ...
  # Filtramos solo Control y PART para la estética rosa
  filter(Group %in% c("Control", "PART")) %>%
  inner_join(top10_with_uniprot, by = c("Protein" = "UNIPROT"))

# 3. Generamos el gráfico con la estética "Rosa"
...
``` r
# Asumiendo que guardaste tu ggplot en un objeto llamado 'p_final'
# Si no, simplemente ejecuta la línea después de tu código de ggplot

ggsave(
  filename = "Figura_Ontologias_Final.png", 
  plot = last_plot(),       # Usa el último gráfico generado
  width = 210,              # Ancho de un A4 (mm)
  height = 297,             # Altura de un A4 (mm) - Esto le da aire para las 5 filas
  units = "mm", 
  dpi = 300,                # Alta resolución para que los puntos no se vean borrosos
